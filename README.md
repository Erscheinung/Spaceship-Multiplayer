...
On September 17, the production `/api/ice` returned an empty server list while the local Metered credentials successfully returned five ICE servers, including TURN. Set the existing private `METERED_DOMAIN` and `METERED_API_KEY` in the Vercel project's Production environment and redeploy. Verify `/api/ice` returns `provider: "metered"` and at least one `turn:` or `turns:` URL; never publish the API key. Credential retrieval alone does not verify relay allocation or cross-network gameplay.
