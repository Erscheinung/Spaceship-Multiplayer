<script>
  ...
  function releaseAction(action){if(engine)engine.actions[action]=false;if(action==='lift')liftHeld=false;}
  function releaseSteering(e){if(e&&e.pointerId!==steeringPointer)return;steeringPointer=null;stick={x:0,z:0};steeringBoost=false;boostHeld=false;if(engine&&controlMode==='drag'){engine.touch=stick;engine.actions.boost=false;}}
